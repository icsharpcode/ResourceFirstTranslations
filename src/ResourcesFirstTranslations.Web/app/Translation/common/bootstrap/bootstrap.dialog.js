(function () {
    ...
    function modalDialog($modal, $templateCache) {
        ...
        $templateCache.put('modalResultDialog.tpl.html',
          '<div>' +
          '    <div class="modal-header">' +
          '        <button type="button" class="close" data-dismiss="modal" aria-hidden="true" data-ng-click="cancel()">&times;</button>' +
          '        <h3>{{title}}</h3>' +
        ...
        function resultDialog(title, msg, okText) {
            var modalOptions = {
                templateUrl: 'modalResultDialog.tpl.html',
                controller: ModalResultInstance,
                keyboard: true,
                resolve: {
                    options: function () {
                        return {
    ...
    var ModalResultInstance = ['$scope', '$modalInstance', 'options',
     function ($scope, $modalInstance, options) {
         $scope.title = options.title || 'Title';
         $scope.message = options.message || '';
         $scope.okText = options.okText || 'OK';
